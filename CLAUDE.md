...
We're building the app described in `.claude/specs/SPEC.md`. Read that file for general architectural tasks or to double-check the exact database structure, tech stack or application architecture. Do not consider any other file within the `.claude/specs/other` folder as a legitimate source of information; those files are only meant to provide context and lessons learned during the development of the app.
